...
需要用到 rpmbuild 命令加上 --rebuild 参数。如 rpmbuild --rebuild `***.src.rpm` 。然后在 `/usr/src/` 下找

5. FC3下iso程序安装： system-config-packages --isodir=iso <br>

RH下iso程序安装： redhat-config-packages --isodir=iso <br>
...
(1) 解压： tar -xvzf 文件.tar.gz <br>

(2) tar.gz解至tar： gzip -d 文件.tar.gz <br>
...
4. bz2类： (1) 解压： bunzip2 文件.bz2 或 bzip2 -d 文件.bz2； (2) 压缩： bzip2 待压缩文件
